  ...
  return (
    <div className="min-h-screen bg-[#070a12] pt-24 pb-16 animate-fade-in text-slate-100 relative overflow-hidden selection:bg-sky-500/30">
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 mb-8">
              ...
              <div 
                ...
                className={`p-4 sm:p-5 rounded-3xl border cursor-pointer transition-all duration-300 relative overflow-hidden backdrop-blur-xl ${
              ...
              >
                ...
                <p className="text-2xl sm:text-4xl font-black text-white tracking-tight">{pendingOrders.length}</p>
                <p className="text-[10px] sm:text-[11px] text-amber-200/70 mt-1">Awaiting manager decision</p>
              </div>
              ...
              >
                ...
                <p className="text-2xl sm:text-4xl font-black text-white tracking-tight">{inBayOrders.length}</p>
                <p className="text-[10px] sm:text-[11px] text-sky-200/70 mt-1">Undergoing live repair</p>
              </div>
              ...
              >
                <div className="flex items-center justify-between text-xs text-rose-300 font-bold mb-2 uppercase tracking-wider">
                  ...
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-rose-500/20 text-rose-300 border border-rose-500/30">
                    {selectedStageFilter === 'URGENT' ? 'Filtered' : 'Priority'}
                  </span>
                </div>
                <p className="text-2xl sm:text-4xl font-black text-white tracking-tight">{urgentOrders.length}</p>
                <p className="text-[10px] sm:text-[11px] text-rose-200/70 mt-1">
                  {selectedStageFilter === 'URGENT' ? 'Viewing urgent fleet' : 'Filter express priority'}
                </p>
              </div>
              ...
              >
                ...
                <p className="text-2xl sm:text-4xl font-black text-white tracking-tight">{readyOrders.length}</p>
                <p className="text-[10px] sm:text-[11px] text-yellow-200/70 mt-1">Awaiting client pickup</p>
              </div>

              {/* 4. Delivered History (Spans 2 cols on mobile) */}
              <div 
                onClick={() => setSelectedStageFilter(selectedStageFilter === 'DELIVERED' ? 'ALL' : 'DELIVERED')}
                className={`col-span-2 sm:col-span-1 lg:col-span-1 p-4 sm:p-5 rounded-3xl border cursor-pointer transition-all duration-300 relative overflow-hidden backdrop-blur-xl ${
              ...
              >
                ...
                <p className="text-2xl sm:text-4xl font-black text-white tracking-tight">{deliveredOrders.length}</p>
                <p className="text-[10px] sm:text-[11px] text-emerald-200/70 mt-1">Archived delivered logs</p>
              </div>
            </div>

            {/* SEARCH & STAGE FILTERS BAR */}
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-2">
              {/* Filter Pills with Color Coding (Horizontally Scrollable on Mobile) */}
              <div className="flex items-center gap-1.5 overflow-x-auto w-full sm:w-auto p-1.5 bg-[#0a0f1d]/80 border border-white/10 rounded-2xl backdrop-blur-xl shrink-0">
                {[
      ...
      {showIntelligencePanel && (
        <IntelligencePanel onClose={() => setShowIntelligencePanel(false)} bookings={bookings} />
      )}
